...
RSpec.describe 'invoices show' do
  describe "when I visit the merchant show page it" do
    ...
    it "shows a link to the applied discount next to each item" do
      visit "/merchant/#{@merchant1.id}/invoices/#{@invoice1.id}"
    ...
    it "applies the best possible discount if the item meets multiple discounts requirements" do
      visit "/merchant/#{@merchant1.id}/invoices/#{@invoice1.id}"
      ...
      within(".table") do
        within("#the-status-#{@ii4.id}") do
          expect(page).to have_link("Discount ID##{discount5.id}")
        end
      end
    end

    it "does not apply a discount if the requierments are not met" do
      visit "/merchant/#{@merchant1.id}/invoices/#{@invoice1.id}"
    ...
    it "redirects me to the discount's show page when it's link is clicked" do
      visit "/merchant/#{@merchant1.id}/invoices/#{@invoice1.id}"
